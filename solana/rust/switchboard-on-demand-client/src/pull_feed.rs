...
impl PullFeed {
    ...
    pub async fn fetch_batch_update_ixs(
        context: Arc<SbContext>,
        client: &RpcClient,
        params: FetchUpdateBatchParams,
        ...
        let mut all_submissions: Vec<Vec<Submission>> =
            vec![Default::default(); params.feeds.len()];
        ...
        let oracle_keys: Vec<Pubkey> = price_signatures
            ...
            .collect();
        for (idx, submissions) in all_submissions.into_iter().enumerate() {
            let feed = params.feeds[idx];
