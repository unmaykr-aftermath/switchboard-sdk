...
export async function fetchCustomClient<Req, Res>(
...
const example = async () => {
  console.log(
    "This example demonstrate how one can config for a custom client to be used by the SDK"
  );

  async function withSuperagentClient() {
    const config = new AptosConfig({
      network: APTOS_NETWORK,
      client: { provider: superagentCustomClient },
    });
  ...
  async function withFetchClient() {
    const config = new AptosConfig({
      network: APTOS_NETWORK,
      client: { provider: fetchCustomClient },
    });
    const aptos = new Aptos(config);
